...
class Dataset(data.Dataset):
    ...
    def generate_triple(self, x_y, num_triples):
        ...
        for i in range(num_triples):
            ...
            keys_sample_list.append(keys_sample)

        # x0 x1
        for i in range(num_triples):

            keys_sample = keys_sample_list[i]
            key_p = keys_sample[0]
                        
            bool_p = x_y[key_p][:,-1] > 0
            bool_n = x_y[key_p][:,-1] == 0
            doc_p = x_y[key_p][bool_p]
            doc_n = x_y[key_p][bool_n]
                        
            if doc_p.shape[0] == 0:

                index = np.random.choice(np.arange(doc_n.shape[0]), 2, replace=False)
                
                tmp_x0.append(doc_n[index[0].item(), :-1])
                tmp_y0.append(doc_n[index[0].item(), -1])
                
                tmp_x1.append(doc_n[index[1].item(), :-1])
                tmp_y1.append(doc_n[index[1].item(), -1])
                
                continue
            
            if doc_n.shape[0] == 0:

                index = np.random.choice(np.arange(doc_p.shape[0]), 2, replace=False)
                
                tmp_x0.append(doc_p[index[0].item(), :-1])
                tmp_y0.append(doc_p[index[0].item(), -1])
                
                tmp_x1.append(doc_p[index[1].item(), :-1])
                tmp_y1.append(doc_p[index[1].item(), -1])
                                
                continue
            
            if i % 2 == 0:            
            
                index_p = np.random.choice(np.arange(doc_p.shape[0]), 1, replace=False)

                tmp_x0.append(doc_p[index_p.item(), :-1])
                tmp_y0.append(doc_p[index_p.item(), -1])
                
            else:
                
                index_n = np.random.choice(np.arange(doc_n.shape[0]), 1, replace=False)

                tmp_x0.append(doc_n[index_n.item(), :-1])
                tmp_y0.append(doc_n[index_n.item(), -1])
                
                
            index_n = np.random.choice(np.arange(doc_n.shape[0]), 1, replace=False)

            tmp_x1.append(doc_n[index_n.item(), :-1])
            tmp_y1.append(doc_n[index_n.item(), -1])
...
def pair_actor_loss(actor_output_1, actor_output_2, actor_output_3, selection_1, selection_2, critic_loss, baseline_loss, lamda, margin):
    ...
    #*************************************************************************
    Reward = critic_loss.detach() - baseline_loss.detach()
    ...
    custom_actor_loss = (Pi_1 + Pi_2)/float(2) * Reward + lamda * (L0_1 + L0_2)/float(2)
    #***************************************************************************
...
def train_model(actor_model, critic_model, baseline_model, patience, epoch_start_early_stopping, saved_path, epochs, lamda, margin):
    ...
    for epoch in range(epochs):
        ...
        for batch, (data1, y1, data2, y2, data3, y3) in enumerate(train_loader):
            ...
            # train critic model
            critic_output_00 = critic_model(data1, selection_1, data2, selection_2)
            label_difference_00 = y1.gt(y2).double()
            critic_loss_output_1 = critic_criterion(critic_output_00.view(1, -1)[0].double(), label_difference_00)
            ...
            baseline_output_00 = baseline_model(data1, data2)
            label_difference_00 = y1.gt(y2).double()
            baseline_loss_output_1 = baseline_criterion(baseline_output_00.view(1, -1)[0].double(), label_difference_00)
            ...
            actor_loss_output = pair_actor_loss(actor_output_1, actor_output_2, actor_output_3, selection_1, selection_2, critic_loss_output, baseline_loss_output, lamda, margin)
        ...
        with torch.no_grad():   
            for batch, (data1, y1, data2, y2, data3, y3) in enumerate(vali_loader):
                ...
                #critic model
                critic_output_00 = critic_model(data1, vali_selection_1, data2, vali_selection_2)
                label_difference_00 = y1.gt(y2).double()
                vali_critic_loss_output = critic_criterion(critic_output_00.view(1, -1)[0].double(), label_difference_00)
                # basseline model
                baseline_output_00 = baseline_model(data1, data2)
                label_difference_00 = y1.gt(y2).double()
                vali_baseline_loss_output = baseline_criterion(baseline_output_00.view(1, -1)[0].double(), label_difference_00)
                ...
                vali_actor_loss_output = pair_actor_loss(vali_actor_output_1, vali_actor_output_2, vali_actor_output_3, vali_selection_1, vali_selection_2, vali_critic_loss_output, vali_baseline_loss_output, lamda, margin)
