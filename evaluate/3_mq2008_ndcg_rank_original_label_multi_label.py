...
def get_qrel_run_predictor(k, option):
    ...
    for qid in x_y.keys():
        ...
        critic_output = torch.max(critic_output, dim=1)[1]

        y_ture = x_y[qid][:,-1].type(torch.float).numpy()
        y_score = critic_output.detach().numpy()
...
def get_qrel_run_baseline(k):
    ...
    for qid in x_y.keys():
        ...
        baseline_output = torch.max(baseline_output, dim=1)[1]
        
        y_ture = x_y[qid][:,-1].type(torch.float).numpy()
        y_score = baseline_output.detach().numpy()
